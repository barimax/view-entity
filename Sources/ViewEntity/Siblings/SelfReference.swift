...
public final class SelfReference<T: EntityModelProtocol>: OptionableEntityProtocol where T: OptionableEntityProtocol {
    ...
    public static func options(database db: Database) async throws -> [Option] {
        return try await T.options(database: db)
    }
    public static func backRefsOptions(fieldName: String, id: UUID, database db: Database) async throws -> [Option] {
    ...
    public static func customOptions(request: Request) async throws -> [String:[Option]]? { nil }
}
